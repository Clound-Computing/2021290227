...
class MyDataset(Dataset):
    def __init__(self, data, VOCAB, max_sen_len, transform_vgg=None, transform_dct=None):
        ...
        self.post_id = torch.from_numpy(data['ID'])
        self.tweet_content = data['text']
        # self.image = list(self.transform(data['image']))
        self.image = list(data['image_id'])
        self.label = torch.from_numpy(data['label'])  # type:int

    def __getitem__(self, idx):
        content = str(self.tweet_content[id])
...
class TrainALL(tune.Trainable):
    def get_dataloader(self):
        ...
        import data_process as pro # type: ignore
    ...
    def setup(self, config):
        ...
        self.CASED = 'E:/vf/ython/cloud/final/bert-base-multilingual-cased/'  # multilingual-cased
        self.VOCAB = 'E:/vf/ython/cloud/final/bert-base-multilingual-cased/vocab.txt'
        self.pthfile = 'E:/vf/ython/cloud/final/vgg19-dcbb9e9d.pth'
    ...
    def print_result_table_handler(self, loss_values, acc_values,
        ...
        # 这个地方需要绝对路径，不能使用相对路径
        if not os.path.exists('E:/vf/ython/cloud/final/results'):
            os.makedirs('E:/vf/ython/cloud/final/results')
        f_result = open(
            'E:/vf/ython/cloud/final/results/{}_accuracy.txt'.format(
                self.dataset_name), 'a+')
...
if __name__ == '__main__':
    ...
    analysis = tune.run(
        ...
        config={
            ...
            "dataset_name": tune.grid_search(['experiment']),
